...
class ExportGaussianSplat(Exporter):
    ...
    def write_ply(
        ...
        antialiased: typing.Optional[bool] = None,
    ):
        """
        ...
        antialiased (Optional[bool]): Whether the model was trained with the
            antialiased (mip-splatting) rasterize mode. When not None, a
            'SplatRenderMode' header comment is written ('mip' when antialiased,
            'default' otherwise) so viewers can select a matching rasterization
            mode. When None (default), no such comment is written.
        """
        ...
        with open(filename, "wb") as ply_file:
            ...
            # Record the rasterization mode so viewers can match it. 'mip' is the
            # antialiased mode; 'default' is classic. See SplatfactoModelConfig.
            if antialiased is not None:
                render_mode = "mip" if antialiased else "default"
                ply_file.write(f"comment SplatRenderMode: {render_mode}\n".encode())
    ...
    def main(self) -> None:
        ...
        # Record whether the model uses the antialiased (mip-splatting) rasterize
        # mode so the exported PLY carries a matching SplatRenderMode comment.
        antialiased = model.config.rasterize_mode == "antialiased"
        ExportGaussianSplat.write_ply(str(filename), count, map_to_tensors, antialiased=antialiased)
